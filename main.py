...
def pdetails():
    ...
    print(code)
    data=Bookingpatient.query.filter_by(email=code).first()
...
def slotbookig():
    ...
    if request.method=="POST":
        ...
        if check!=None:
            if(seat>0 and check):
                res=Bookingpatient(bedtype=bedtype,hcode=hcode,spo2=spo2,pname=pname,pphone=pphone,paddress=paddress)
                db.session.add(res)
                db.session.commit()
